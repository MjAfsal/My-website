...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM watche_details WHERE Pro_Id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $watcheId);
    $stmt->execute();
...
<?php
    ...
    $watcheId = $_POST['Pro_Id'];
    $watcheName = $_POST['Pro_Name'];
    $price = $_POST['Price'];
    $quantity = $_POST['Quantity'];


    $query = "UPDATE watche_details SET Pro_Name=?, Price=?, Quantity=? WHERE Pro_Id=?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("sidi", $watcheName, $price, $quantity, $watcheId);
    $stmt->execute();
