...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    $accessorieId = $_GET['id'];

    $query = "SELECT * FROM accessorie_details WHERE Pro_Id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $accessorieId);
    $stmt->execute();
...
<?php
    ...
    $accessorieId = $_POST['Pro_Id'];
    $accessorieName = $_POST['Pro_Name'];
    $price = $_POST['Price'];
    $quantity = $_POST['Quantity'];

    $query = "UPDATE accessorie_details SET Pro_Name=?, Price=?, Quantity=? WHERE Pro_Id=?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("sidi", $accessorieName, $price, $quantity, $accessorieId);
    $stmt->execute();
